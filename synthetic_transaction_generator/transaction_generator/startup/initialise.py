...
from synthetic_transaction_generator.transaction_generator.startup.generate_merchant_ids import generate_merchant_ids
import random
import numpy as np
...
def initialise(config: SimulationConfig) -> tuple[dict[str, CardProfile], list[str]]:
    ...
    merchants = generate_merchant_ids(config.n_merchants)
