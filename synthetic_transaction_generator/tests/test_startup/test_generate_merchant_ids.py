from synthetic_transaction_generator.transaction_generator.startup.generate_merchant_ids import (
    generate_merchant_ids,
)


def test_returns_correct_count():
    recipients = generate_merchant_ids(500)
...
def test_ids_are_well_formed():
    recipients = generate_merchant_ids(10)
...
def test_zero_recipients_returns_empty_list():
    recipients = generate_merchant_ids(0)
    assert recipients == []


def test_large_count_still_zero_padded_correctly():
    recipients = generate_merchant_ids(1_000_001)
